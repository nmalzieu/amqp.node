...
C.open = function(allFields, openCallback0) {
  ...
  function negotiate(server, desired, reversed) {
    ...
    // If `reversed` is true, it is the other way around. Especially
    // usefull for heartbeats, where we should follow the Java Client
    // See https://groups.google.com/forum/#!topic/rabbitmq-users/FpUdTaIr8CI
    // and https://github.com/pika/pika/pull/666
    if (server === 0 || desired === 0) {
      // i.e., whichever places a limit if not reversed, if either
      // or 0 (no limit) else
      return reversed ? 0 : Math.max(server, desired);
    }
    else {
      return reversed ? Math.max(server, desired) : Math.min(server, desired);
    }
  }
  ...
  function afterStartOk(reply) {
    ...
    case defs.ConnectionTune:
      ...
      tunedOptions.heartbeat =
        negotiate(fields.heartbeat, allFields.heartbeat, true);
